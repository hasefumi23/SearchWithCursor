...
const URL_REGEX: RegExp = /(?:https?:\/\/|localhost|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>\[\]]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>\[\]]+|(\([^\s()<>\[\]]+\)))*\)|[^\s`!(){}\[\];:'".,<>?«»“”‘’])/gi;

export function activate(context: vscode.ExtensionContext) {
	let disposable = vscode.commands.registerCommand(
		"extension.searchWithCursor",
		() => {
...
const pickUrl = (text: string): string => {
	...
	const searchEngine = vscode.workspace
		.getConfiguration("searchWithCursor")
		.get("customSearchEngine", "https://www.google.com/search?q=%s");
	return searchEngine.replace(/%s/g, encodedText);
}

const getText = (editor: vscode.TextEditor): string => {
